  ...
  const DesktopForm = () => (
    <div className="hidden md:block">
      <form className="space-y-10" onSubmit={handleSubmit}>
          ...
          <div className="space-y-2">
            {files.map((file, i) => (
              <div key={i} className="flex items-center gap-3 bg-surface-container-low px-4 py-3">
                <Image className="w-4 h-4 text-primary shrink-0" />
                <span className="text-sm truncate flex-1">{file.name}</span>
                <button type="button" onClick={() => removeFile(i)} className="text-secondary hover:text-foreground">
                  <X className="w-4 h-4" />
                </button>
              </div>
            ))}
        ...
        <button className="w-auto bg-primary text-primary-foreground font-headline uppercase tracking-widest text-sm px-12 py-5 hover:bg-primary-container transition-all duration-150 flex items-center gap-3" type="submit">
